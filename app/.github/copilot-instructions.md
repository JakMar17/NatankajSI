...
* **Explanations:** When generating code, provide explanations for hard to grasp Dart-specific
...
* **State Management:** Separate ephemeral state and app state. Use a state
  management solution for app state to handle the separation of concerns. Use BLoC and Cubit pattern for state management (prefer `Cubit` for simpler state management needs).
* **Widgets are for UI:** Everything in Flutter's UI is a widget. Compose
  complex UIs from smaller, reusable widgets.
* **Stateless vs Stateful:** Always prefer `StatelessWidget` when possible, when state is needed, use BLoC/Cubit instead of `StatefulWidget` to keep UI code clean and maintainable.
* **One Class per File:** Follow the convention of one class per file for better readability and maintainability, if classes are closely related, they can be linked via a shared private library file (part/part of).
* **One line methods:** if method/function body is a single expression, use the arrow syntax (`=>`).
...
### State Management
* **Built-in Solutions:** For state management use BLoC and Cubit pattern for state management (prefer `Cubit` for simpler state management needs).
...
For data serialization and deserialization do not use automatic code generation. Instead, write manual `fromJson` and `toJson` methods for each data class. This approach provides better control over the serialization process and avoids the complexity of code generation.
